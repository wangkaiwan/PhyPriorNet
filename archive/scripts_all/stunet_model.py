"""Standalone STU-Net (3D) extracted from the HECKTOR project's STUNetTrainer.py
(HECKTOR2025-MEDAI/STU-Net). Pure torch nn.Module, no nnU-Net dependency — usable in our
cyclegan env for sCT regression. Deep supervision is built in (forward returns a tuple of
multi-resolution outputs when decoder.deep_supervision=True).

For sCT regression: input_channels=1, num_classes=1, final output passed through sigmoid externally
to map to [0,1] (STU-Net's final_nonlin is identity)."""
import torch
...
def build_stunet(variant="base", in_ch=1, out_ch=1, deep_supervision=True):
    """sCT STU-Net for patch ~192x192x64. Anisotropic pooling so the thin z-axis (64) isn't
    over-downsampled: in-plane /32 (192->6), z /8 (64->8). 6 levels, 5 pooling ops."""
    dims_mult = {"small": 16, "base": 32}[variant]
    dims = [dims_mult * x for x in [1, 2, 4, 8, 16, 16]]
    pool = [[2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 1], [2, 2, 1]]
    kernels = [[3, 3, 3]] * 6
    return STUNet(in_ch, out_ch, depth=[1] * 6, dims=dims,
                  pool_op_kernel_sizes=pool, conv_kernel_sizes=kernels,
